...
import { formatAge, formatDateTime, formatDuration, formatDate } from "@/utils/date";
...
export default function ShareView() {
  ...
  const stats = useMemo(() => {
    ...
    return {
      milkCount: payload.feedings.filter((f) => f.type === "milk").length,
      milkAmount: payload.feedings.filter((f) => f.type === "milk").reduce((s, f) => s + f.amount, 0),
      solidsCount: payload.feedings.filter((f) => f.type === "solids").length,
      diaperCount: payload.diapers.length,
      sleepCount: payload.sleeps.length,
    };
  }, [payload]);
  ...
  const { baby, date, feedings, diapers, sleeps, totalSleepMin, todos, latestGrowth, vaccines } =
    payload;
